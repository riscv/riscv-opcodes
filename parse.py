...
from shared_utils import create_inst_dict, add_segmented_vls_insn
...
def generate_outputs(instr_dict, extensions):
    ...
    extension_map = {
        '-c': {
            'function': lambda: make_c(collections.OrderedDict(sorted(create_inst_dict(extensions, False, include_pseudo_ops=emitted_pseudo_ops).items()))),
