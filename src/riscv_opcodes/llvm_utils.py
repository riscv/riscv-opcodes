...
from .shared_utils import CsrDict, InstrDict, SingleInstr, read_extension_file
...
def _render_properties(properties: "dict[str, object]") -> str:
    if not properties:
        return ""
...
def _indent_block(text: str, prefix: str = "    ") -> str:
    return "".join(
        prefix + line if line else line for line in text.splitlines(keepends=True)
    )
    ...
    def from_str(cls, type_str: str) -> "DataType":
        for label in cls:
            if type_str == label.value:
                return cls(label)
        raise ValueError(
            f"DataType: cannot recognize data type from string: '{type_str}'"
        )
...
class Instruction:
    ...
    def from_dict(cls, mnemonic: str, spec: SingleInstr) -> "Instruction":
        ...
        print(
            f"Instruction {mnemonic}: operands {spec['variable_fields']} -> format {fmt}"
        )
...
def _get_properties(mnemonic: str) -> "dict[str, object]":
    props: "dict[str, object]" = {"hasSideEffects": 0, "mayLoad": 0, "mayStore": 0}
...
# Maps 7-bit opcode bit strings to the existing OPC_* def names in
# RISCVInstrFormats.td. Using shared defs avoids per-instruction RISCVOpcode
# records, whose Name field is constrained to char Name[10].
_RISCV_OPCODE_DEFS = {
    "0b0000011": "OPC_LOAD",
    "0b0000111": "OPC_LOAD_FP",
    "0b0001011": "OPC_CUSTOM_0",
    "0b0001111": "OPC_MISC_MEM",
    "0b0010011": "OPC_OP_IMM",
    "0b0010111": "OPC_AUIPC",
    "0b0011011": "OPC_OP_IMM_32",
    "0b0100011": "OPC_STORE",
    "0b0100111": "OPC_STORE_FP",
    "0b0101011": "OPC_CUSTOM_1",
    "0b0101111": "OPC_AMO",
    "0b0110011": "OPC_OP",
    "0b0110111": "OPC_LUI",
    "0b0111011": "OPC_OP_32",
    ...
    "0b1010011": "OPC_OP_FP",
    "0b1010111": "OPC_OP_V",
    "0b1011011": "OPC_CUSTOM_2",
    "0b1100011": "OPC_BRANCH",
    "0b1100111": "OPC_JALR",
    "0b1101111": "OPC_JAL",
    "0b1110011": "OPC_SYSTEM",
    "0b1111011": "OPC_CUSTOM_3",
}


def _opcode_ref(opcode_bits: str, instr_name: str) -> str:
    """Return the TableGen expression for an opcode argument.
    Uses an existing OPC_* def when available; falls back to an inline
    RISCVOpcode<> with a truncated name that fits char Name[10]."""
    if opcode_bits in _RISCV_OPCODE_DEFS:
        return _RISCV_OPCODE_DEFS[opcode_bits]
    # Fallback: inline record — truncate name to 9 chars to fit char Name[10]
    short = instr_name[:9]
    return f'RISCVOpcode<"{short}", {opcode_bits}>'
...
def _tblgen_def(inst: Instruction, ext_name: str) -> str:
    ...
    if fmt == InstructionFormat.RPRS3:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs ),\n"
    ...
    if fmt == InstructionFormat.RIMM5:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs {dtype['rd']}:$rd),\n"
    ...
    if fmt == InstructionFormat.RIMM5_RS1Z:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs {dtype['rd']}:$rd),\n"
    ...
    if fmt == InstructionFormat.RIMM6:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs {dtype['rd']}:$rd),\n"
    ...
    if fmt == InstructionFormat.R_RS1Z:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs {dtype['rd']}:$rd),\n"
    ...
    if fmt == InstructionFormat.R_RDZ_RS2Z:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs ),\n"
    ...
    if fmt == InstructionFormat.IIMM12:
        if inst.mnemonic in FP_MEM_LOAD_MNEMONICS:
            return (
                ...
                f"                {_opcode_ref(e.opcode, tblgen_name)},\n"
                f"                (outs {dtype['rd']}:$rd),\n"
                f"                (ins GPRMem:$rs1, simm12_lo:$imm12),\n"
        ...
        return (
            ...
            f"                {_opcode_ref(e.opcode, tblgen_name)},\n"
            f"                (outs {dtype['rd']}:$rd),\n"
            f"                (ins {dtype['rs1']}:$rs1, simm12_lo:$imm12),\n"
            f'                "{mnemonic}", "$rd, $rs1, ${{imm12}}(${{rs1}})">,\n'
            f"                Sched<[]>;\n"
        )
    if fmt == InstructionFormat.IIMM12_RM:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs {dtype['rd']}:$rd),\n"
            f"                (ins {dtype['rs1']}:$rs1, simm12_lo:$imm12, frmarg:$frm),\n"
            ...
            f"               ({tblgen_name} {dtype['rd']}:$rd, {dtype['rs1']}:$rs1, simm12_lo:$imm12, FRM_DYN)>;\n"
        )
    if fmt == InstructionFormat.IIMM12_RS1Z:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs {dtype['rd']}:$rd),\n"
            f"                (ins simm12_lo:$imm12),\n"
            f'                "{mnemonic}", "$rd, $imm12",\n'
            f"                [], InstFormatI>,\n"
            f"                Sched<[]> {{\n"
            f"    bits<5> rd;\n"
    ...
    if fmt == InstructionFormat.IIMM12_RDZ:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs ),\n"
    ...
    if fmt == InstructionFormat.FREP_O:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs ),\n"
            f"                (ins {dtype['rs1']}:$rs1, simm12_lo:$imm12, uimm3:$stagger_max, uimm4:$stagger_mask),\n"
    ...
    if fmt == InstructionFormat.SIMM12:
        if inst.mnemonic in FP_MEM_STORE_MNEMONICS:
            return (
                ...
                f"                {_opcode_ref(e.opcode, tblgen_name)},\n"
                f"                (outs ),\n"
                f"                (ins {dtype['rs2']}:$rs2, GPRMem:$rs1, simm12_lo:$imm12),\n"
        ...
        return (
            ...
            f"                {_opcode_ref(e.opcode, tblgen_name)},\n"
            f"                (outs ),\n"
            f"                (ins {dtype['rs2']}:$rs2, {dtype['rs1']}:$rs1, simm12_lo:$imm12),\n"
            f'                "{mnemonic}", "$rs2, ${{imm12}}(${{rs1}})">,\n'
            f"                Sched<[]>;\n"
        )
    if fmt == InstructionFormat.ISHAMT:
        return (
            f"{props_str}"
    ...
    if fmt == InstructionFormat.ISHAMTW:
        return (
            f"{props_str}"
    ...
    if fmt == InstructionFormat.R_RDZ:
        return (
            f"{props_str}"
            f"def {tblgen_name} : RVInst<\n"
            f"                (outs ),\n"
    ...
    if fmt == InstructionFormat.BIMM12:
        return (
            f"{props_str}"
            ...
            f"                {_opcode_ref(e.opcode, tblgen_name)},\n"
            f"                (outs ),\n"
            f"                (ins {dtype['rs1']}:$rs1, simm5:$imm5, bare_simm13_lsb0:$imm12),\n"
    ...
    if fmt == InstructionFormat.R4FRM:
        return (
            f"{props_str}"
...
def _tblgen_alias(
    ...
    if fmt == InstructionFormat.R_RDZ_RS2Z:
        return (
            f'def : InstAlias<"{mnemonic} $rs1",' f" ({use} {dtype['rs1']}:$rs1), 0>;\n"
        )
    if fmt == InstructionFormat.R_RDZ:
        return (
    ...
    if fmt == InstructionFormat.IIMM12:
        if inst.mnemonic in FP_MEM_LOAD_MNEMONICS:
            return (
                ...
                f" ({use} {dtype['rd']}:$rd, GPRMem:$rs1, simm12_lo:$imm12), 0>;\n"
            )
        return (
            f'def : InstAlias<"{mnemonic} $rd, $rs1, $imm12",'
            f" ({use} {dtype['rd']}:$rd, {dtype['rs1']}:$rs1, simm12_lo:$imm12), 0>;\n"
        )
    if fmt == InstructionFormat.IIMM12_RM:
        return (
            f'def : InstAlias<"{mnemonic} $rd, $rs1, $imm12, frmarg:$frm",'
            f" ({use} {dtype['rd']}:$rd, {dtype['rs1']}:$rs1, simm12_lo:$imm12, frmarg:$frm), 0>;\n"
        )
    if fmt == InstructionFormat.IIMM12_RS1Z:
        return (
            f'def : InstAlias<"{mnemonic} $rd, $imm12",'
            f" ({use} {dtype['rd']}:$rd, simm12_lo:$imm12), 0>;\n"
        )
    if fmt == InstructionFormat.IIMM12_RDZ:
        return (
            f'def : InstAlias<"{mnemonic} $rs1, $imm12",'
            f" ({use} {dtype['rs1']}:$rs1, simm12_lo:$imm12), 0>;\n"
        )
    if fmt == InstructionFormat.FREP_O:
        return (
            f'def : InstAlias<"{mnemonic} $rs1, $imm12, $stagger_max, $stagger_mask",'
            f" ({use} {dtype['rs1']}:$rs1, simm12_lo:$imm12, uimm3:$stagger_max, uimm4:$stagger_mask), 0>;\n"
        )
    if fmt == InstructionFormat.SIMM12:
        if inst.mnemonic in FP_MEM_STORE_MNEMONICS:
            return (
                f'def : InstAlias<"{mnemonic} $rs2, $imm12($rs1)",'
                f" ({use} {dtype['rs2']}:$rs2, GPRMem:$rs1, simm12_lo:$imm12), 0>;\n"
            )
        return (
            f'def : InstAlias<"{mnemonic} $rs2, $rs1, $imm12",'
            f" ({use} {dtype['rs2']}:$rs2, {dtype['rs1']}:$rs1, simm12_lo:$imm12), 0>;\n"
        )
    if fmt == InstructionFormat.ISHAMT:
        return (
...
def make_llvm_features(extensions: Sequence[str]):
    ...
    for ext in extensions:
        ...
        lines.extend(
            [
                f"def {feature_name}",
                f'    : RISCVExtension<1, 0, "{feature_desc}">;',
                f"def {feature_flag}",
