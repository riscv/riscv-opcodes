...
from importlib.resources import contents, is_resource, open_text, read_text
...
def read_lines(package: str, file: str) -> "list[str]":
    """
    Reads lines from a file and returns non-blank, non-comment lines.

    The file must be in a Python package given by `package`.
    """
    with open_text(package, file) as fp:
...
def read_extension_file(ext: str) -> str:
    ...
    """
    if is_resource("riscv_opcodes.extensions", ext):
        return read_text("riscv_opcodes.extensions", ext)
    if is_resource("riscv_opcodes.extensions.unratified", ext):
        return read_text("riscv_opcodes.extensions.unratified", ext)
...
def create_inst_dict(
    ...
    # Package name and file name tuple.
    file_names: list[tuple[str, str]] = []
    for file in contents("riscv_opcodes.extensions"):
        if any(fnmatch(file, fil) for fil in ratified_file_filters):
            file_names.append(("riscv_opcodes.extensions", file))
    for file in contents("riscv_opcodes.extensions.unratified"):
        if any(fnmatch(file, fil) for fil in unratified_file_filters):
            file_names.append(("riscv_opcodes.extensions.unratified", file))

    logging.debug("Collecting standard instructions")
    for package, file_name in file_names:
        logging.debug(f"Parsing File: {file_name} for standard instructions")
        lines = read_lines(package, file_name)
